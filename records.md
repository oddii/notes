...
1. 今天还是继续排查了一下专属语伴昨晚的数据问题，还是会有个别会出现 `event_duration` 报大了的问题，又换了一种统计时间的方式，继续观察数据是否有问题
2. 决定毕设做一个 `Web` 版的网易云音乐了，买了一手 `design+code` 学习一下交互怎么写
3. 进入主 `app 4.2.3` 的需求评审，`iOS` 下一期会修复时间选择器的问题，下期专属语伴项目会使用系统原生的时间选择器

## 2021-01-25 周一

1. 对运营新提的两个 `H5` 需求：动态发布技巧、社区防诈骗指南，进行技术评估，分别使用一天的时间完成
2. 参加 `HL` 历史遗留问题与需求的评审会
3. 在 `single_page` 中新建了一个页面开发动态发布技巧需求，将基本的样式与多语言都完成了，明天需要了解下如何配置入口给运营验收
4. 继续学习 `gatsby`

## 2021-01-26 周二

1. 完成了 `gatsby` 的教程到第八步，第八步是关于打包上线优化的，暂时不需要这么快了解
2. 完成了动态发布技巧的那个需求，但是晚上下班打电话给我说需要做成 `H5` 页面，而不是直接用 `weex` 跳转，明天回来需要重新写一个 `H5` 页面
3. 将社区防诈骗指南也简单开发了一下，多语言也进行了适配
4. 开始学习 `UI Design for Developers`，里面使用 `figma` 进行设计，并用 `figma` 开始画框框

## 2021-01-27 周三

1. 修改动态发布技巧、社区防诈骗指南的多语言逻辑，现在将匹配不到的语言都设置为了英语，这个多语言逻辑较之前会好点
2. 将动态发布技巧、社区防诈骗指南都重新提测了
3. 社区防诈骗指南在运营沟通后，调整了一下样式，运营提出点击了链接后需要一个下划线标示，明天回来实现这个功能，用一下冒泡事件
4. `UI Design for Developers` 学习到了第 5 章，对 `figma` 也越来越熟练

## 2021-01-28 周四

1. 在用 `figma` 的 `Auto Layout` 的时候出现了点问题，`Auto Layout ` 绑定后不能调整里面元素的位置，需要把 `Auto Layout` 解除才可以
2. 专属语伴的时间选择器在 `iOS 4.2.3` 修复后，进行了一波版本适配，但是安卓的选择器头部还是灰色的，有点问题，但是产品说还是使用这个
3. 开始去修复 `Bug`，将课程详情页面-语法内容为空这个 `Bug` 提测了，这个 `Bug` 初步定位是点词翻译的组件里面的 `HTML` 出现了问题，点词翻译这个组件挺骚的，使用 `weex` 里加载 `web` 实现的，而这个 `web` 控件的组件因为 `iOS` 的滑动控件可以弹性上滑的关系在自身的 `overflow: hidden` 后导致了白屏

## 2021-01-29 周五

1. 潜心研究 `figma`，找了许多插图素材的网站，并自己学着搞了一下，对 `Auto Layout` 的理解更深了一点，自己也封装了 `Top Menu` 和 `Slide Menu` 组件
2. 调查了一下 `Bug` 表，排查了一圈发现没有什么能必现的 `Bug`，有些描述信息与日志也丢了

## 2021-02-01 周一

1. 差不多摸了一天的🐟在学习 `figma`，把周末剩下的作业也做了一下，浏览了下 `designcode` 其他关于 `figma` 的课程，感觉有点老，还是在其他地方看一下好了
2. 专属语伴 `1.1.0` 需求评审终于来了，需求量不大，感觉一天就能做完，然后我评估了两天😁
3. 用 `figma` 学习了一下那个磨砂简历，感觉对元素的摆放以及网格的排版都需要费点功夫去学习一下，要多用栅格系统去摆放位置

## 2021-02-02 周二

1. 早上来准备写一下专属语伴 `1.1.0` 的需求，但是笃志说有一个困扰到他凌晨两点的问题没解决，我就去研究那个问题了，那个问题是这样的，现在有一个场景：使用 `React hooks`，需要对一个列表 `Array` 新增一项，并且马上获得这个列表 `Array` 新增成功的那一项的值，注意是从 `Array` 里获取。这个问题就在 `React` 使用 `useState` 的 `set` 方法后，并不会同步更新值，而是在一个渲染帧后，再将所有的 `set` 方法一起更新，所以在 `set` 方法之后马上获取的数组，是在前一个渲染帧中的状态，不会被更新。要想获得更新后的渲染帧，我使用的是 `useEffect` 在副作用中去添加依赖来获取最新的状态，当然笃志研究的那份代码有一个更骚的操作，**以后可以学习一下**
2. 开始做专属语伴 `1.1.0` 的需求，除了获取不到点击日历的日期的位置信息，这个是因为 `native` 中 `weex` 找不到 `v-for` 渲染项的 `ref` ？？其他都基本完成了

## 2021-02-03 周三

1. 早上等公车的时候找了下昨晚那个在 `native` 中 `weex` 找不到 `v-for` 渲染项的 `ref` 的问题，需要使用 `this.$refs.ref[0]` 这样才能拿到渲染项的 `ref`，在解决完这个问题后就用 `figma` 去做数学题解决弹出气泡的位置问题了

2. 下午看到 `gc_web` 群反馈说这个项目的语音播放不了了，跟娜姐沟通了一下是需要重新去调用一个新的借口获得音频，明天再弄一下这个需求

3. 又跟笃志一起梳理了一下 `weex` 那个播放器的问题，然后发现 `Vue` 的 `watch` 操作，如果在一个任务栈中多次更新状态，并用 `watch` 监听，`watch` 只会监听到任务栈顶的那次更新操作，而不会监听全部操作？？如有个方法：
   ...
   如果 `watch` 这个 `msg.a`，那么在 `deep: true, immediate: true` 的情况也，执行这个方法，也只会 `watch` 到两次的操作变化，并且是在栈顶的那次更新操作，而不是监听全部操作

   ```
   1 undefined		->> immediate 的变化
   default: 1		->> immediate 的变化
...
1. 开始继续搞那个 `gc_web` 的语音播放问题，需要重新调整一下接口逻辑，而且在 `nuxt` 中，`vuex` 的 `actions` 中方法的第一个参数结构出来的 `commit`、`dispatch`，只能找到本文件内的方法，不能使用其他文件的方法，如果要使用其他文件的方法要同 `this.$store.commit` 这样
2. 又是学习 `figma` 的一天

## 2021-02-05 周五

1. `gc_web` 那个语音播放原来是要点击才播放，所以又要调整一下接口逻辑，还要调整下交互逻辑
2. 开表彰大会摸了一下午的🐟，准备用 `figma` 搞一下语伴天天练的广告图，但是发现好难搞

## 2021-02-07 周日

1. `gc_web` 上次调整的语音问题上线到正式环境了
2. 用 `figma` 搞了一张有年味儿的语伴天天练广告，是抄快手一个设计师的

## 2021-02-22 周一

1. 重新学习 `gatsby`，并上了 `designcode` 的十节课，但是感觉还是讲的太浅了，后续可能还需要听一下其他老师的相关课程
2. 修改了一下语伴天天练的部分文案

## 2021-02-23 周二

1. 将 `designcode` 剩下的十节相关 `gatsby` 的看完了
2. 对邮件购买 `1.1.0` 版本进行需求评估
3. 向人事与头头提出离职申请
4. 找了一套原型国旗，将全部国家除 `AC` 外，全部上传到了 `cdn`，方便语伴天天练添加新的老师，就不用每次添加老师找开发加新的国旗了

## 2021-02-24 周三

1. 语伴天天练全套国旗除 `AC` 外上线到正式环境
2. 开始做邮件购买的 `1.1.0` 需求
3. 参与课程业务2021目标讨论会，开了两个多小时，被张凯 cue 了两次

## 2021-02-25 周四

1. 完成邮件购买 `1.1.0` 需求的前端部分，并未能开始前后端联调
2. 写工作交接文档
3. 与谢为对语伴天天练已过期用户显示问题进行联调修复
4. 开始准备毕设的主题色
